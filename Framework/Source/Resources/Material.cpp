...
namespace fw {
    ...
    void Material::Enable(const Uniforms* pUniforms) const
    {
        ...
        if( m_pTexture )
        {
            bgfx::setTexture( 0, pUniforms->GetUniform("u_TextureColor"), m_pTexture->GetHandle() );
        }

        // UV scale and offset.
        bgfx::setUniform( pUniforms->GetUniform( "u_UVScale" ), &m_UVScale.x );
        bgfx::setUniform( pUniforms->GetUniform( "u_UVOffset" ), &m_UVOffset.x );

        // Vertex Colors.
        bgfx::setUniform( pUniforms->GetUniform( "u_DiffuseColor" ), &m_Color.r );
    }
